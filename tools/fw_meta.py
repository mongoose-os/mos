...
def cmd_gen_build_info(args):
    ...
    if args.version:
        ...
        if (
            args.tag_as_version
            and git_describe_out
            and "-" not in git_describe_out
    ...
    if args.id:
        ...
        if git_describe_out:
            ...
            if head_hash not in git_describe_out or git_describe_out.startswith(
                head_hash
            ):
    ...
    for js_file in args.js_files:
        with open(js_file, encoding="utf-8") as f:
            ...
            for m in re.finditer(
                r"""\bffi\s*\(\s*(?P<quote>['"])(?P<symbol_type>[^)]+?\W)(?P<symbol_name>\w+)(?P<args>.+?)(?P=quote)""",
                data,
            ):
                symbols.append(
                    FFISymbol(
                        m.group("symbol_name"), m.group("symbol_type"), m.group("args")
                    )
                )
...
def stage_file_and_calc_digest(args, part, fname, staging_dir):
    with open(fname, "rb") as f:
        ...
        part["size"] = len(data)
        if staging_dir:
            staging_file = os.path.join(staging_dir, os.path.basename(fname))
        ...
        if args.checksums and "src" in part:
            ...
            if os.path.isfile(src):
                ...
                for fname in os.listdir(src):
                    file_attrs = {}
                    stage_file_and_calc_digest(
                        args, file_attrs, os.path.join(src, fname), staging_dir
                    )
